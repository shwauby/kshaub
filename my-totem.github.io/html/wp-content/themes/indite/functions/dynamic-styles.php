...
/* ------------------------------------ */
if ( ! function_exists( 'indite_hex2rgb' ) ) {

	function indite_hex2rgb( $hex, $array=false ) {
		$hex = str_replace("#", "", $hex);
...
/*  Google fonts
/* ------------------------------------ */
if ( ! function_exists( 'indite_enqueue_google_fonts' ) ) {

	function indite_enqueue_google_fonts () {
...
add_action( 'wp_enqueue_scripts', 'indite_enqueue_google_fonts' ); 	


/*  Dynamic css output
/* ------------------------------------ */
if ( ! function_exists( 'indite_dynamic_css' ) ) {

	function indite_dynamic_css() {
		if ( get_theme_mod('dynamic-styles', 'on') == 'on' ) {
		
			// rgb values
			$color_1 = get_theme_mod('color-1');
			$color_1_rgb = indite_hex2rgb($color_1);
			...
			// primary color
			if ( get_theme_mod('color-1','#32c1fb') != '#32c1fb' ) {
				$styles .= '
#header,
#profile-image { border-top-color: '.esc_attr( get_theme_mod('color-1') ).'; }
.entry .more-link { border-left-color: '.esc_attr( get_theme_mod('color-1') ).'; }
				'."\n";
			}
			// secondary color
			if ( get_theme_mod('color-2','#8c53ff') != '#8c53ff' ) {
				$styles .= '
.s2 { border-top-color: '.esc_attr( get_theme_mod('color-2') ).'; }
#profile-image { border-right-color: '.esc_attr( get_theme_mod('color-2') ).'; }
.entry .more-link { border-top-color: '.esc_attr( get_theme_mod('color-2') ).'; }
				'."\n";
			}
			// link color
...
.entry a { color: '.esc_attr( get_theme_mod('color-link') ).'; }
			...
			wp_add_inline_style( 'indite-style', $styles );	
		}
	}
	
}
add_action( 'wp_enqueue_scripts', 'indite_dynamic_css' );
