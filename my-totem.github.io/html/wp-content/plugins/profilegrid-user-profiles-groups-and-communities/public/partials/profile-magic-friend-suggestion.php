...
if(empty($suggestions)){
    echo '<p class="pm-no-result-found">'.__('No suggestions available for friends','profilegrid-user-profiles-groups-and-communities').'</p>';
}else{
...
<input type="submit" name="pm_u_search_button" id="pm_u_search_button" value="<?php _e('Search','profilegrid-user-profiles-groups-and-communities');?>" />
</form>
</div>
...
<button id="pm_load_more_suggestion" value="2" onclick="pm_load_more_suggestion(<?php echo $uid;?>,this.value,<?php echo $num_of_suggestions_pages;?>)"><?php _e('Load More Result','profilegrid-user-profiles-groups-and-communities');?></button>
