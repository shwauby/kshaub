...
//        $identifier = 'GROUPS';
//        $pm_activator = new Profile_Magic_Activator;
//        $group_table = $pm_activator->get_db_table_name($identifier);
//        
//        $qry = "SELECT count(u.ID) as members FROM wp_users u 
//        LEFT JOIN wp_usermeta  um1 ON u.ID = um1.user_id
//        WHERE  um1.meta_key = 'pm_group' AND um1.meta_value LIKE '%:\"1\";%'"; 
//        
//        
//        @qry2 = "select g.*,@a:=( SELECT count(u.ID) as members FROM wp_users u LEFT JOIN wp_usermeta um1 ON u.ID = um1.user_id WHERE um1.meta_key = 'pm_group' AND um1.meta_value LIKE concat('%:\"g.id\";%') from wp_promag_groups g";
//        $qry2 = "select $group_table.* from $group_table"
//        return $user_query;
    }
