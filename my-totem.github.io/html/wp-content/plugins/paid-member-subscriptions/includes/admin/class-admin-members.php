...
Class PMS_Submenu_Page_Members extends PMS_Submenu_Page {
    ...
    private function get_log_message( $log ) {
        ...
        switch ( $log['type'] ) {
            ...
            case 'subscription_upgrade_attempt':
                $message = sprintf( __( 'User attempted to upgrade his subscription to <strong>%s</strong>.', 'paid-member-subscriptions' ), $this->get_subscription_plan_name( $log['data']['new_plan'] ) );
                break;
            ...
            case 'subscription_upgrade_success':
                $message = sprintf( __( 'Subscription upgraded from <strong>%s</strong> to <strong>%s</strong>.', 'paid-member-subscriptions' ), $this->get_subscription_plan_name( $log['data']['old_plan'] ), $this->get_subscription_plan_name( $log['data']['new_plan'] ) );
                break;
            case 'subscription_trial_started':
                $message = sprintf( __( 'Subscription trial started until <strong>%s</strong>.', 'paid-member-subscriptions' ), pms_sanitize_date( $log['data']['until'] ) );
                break;
            ...
            case 'admin_subscription_edit':
                ...
                $message = sprintf( __( '%s changed <strong>%s</strong> from <strong>%s</strong> to <strong>%s</strong>.', 'paid-member-subscriptions' ), $admin_name, $field, !empty( $log['data']['old'] ) ? $this->get_subscription_plan_name( $log['data']['old'], $field ) : '-', !empty( $log['data']['new'] ) ? $this->get_subscription_plan_name( $log['data']['new'], $field ) : '-' );
                break;
    ...
    // Given a subscription plan id, returns its name
    private function get_subscription_plan_name( $plan_id, $field = '' ){
