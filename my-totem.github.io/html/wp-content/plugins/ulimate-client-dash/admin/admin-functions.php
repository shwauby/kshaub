...
add_action('admin_menu', function() {
    add_menu_page( 'Ultimate Client Dash settings', __( 'Ultimate Client', 'ultimate-client-dash' ), 'administrator', 'ultimate-client-dash', 'ucd_client_dash_page', 'dashicons-dashboard', 90  );
    ...
    add_submenu_page( 'ultimate-client-dash', 'ucd_dashboard_options_page', __( 'Custom Code', 'ultimate-client-dash' ), 'administrator', 'ultimate-client-dash&tab=tracking-and-custom-code', 'ucd_client_dash_page' );
    ...
    // Show Buy Pro Version if pro version is not installed
		...
		if (is_plugin_active('ultimate-client-dash-pro/ultimate-client-dash-pro.php') ) {
		  	add_submenu_page( 'ultimate-client-dash', 'ucd_dashboard_options_page', __( 'License', 'ultimate-client-dash' ), 'administrator', 'ultimate-client-dash&tab=activate', 'ucd_client_dash_page');
		}
...
function ucd_custom_save_notice() {
    global $pagenow;
    if ($pagenow == 'admin.php') {
        if ( isset( $_GET['settings-updated'] )) {
            add_settings_error( 'ucd-notices', 'ucd-settings-updated', __('Settings saved.', 'ucd'), 'updated' );
        }
    }
}
add_action('admin_notices', 'ucd_custom_save_notice');
