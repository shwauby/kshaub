...
<div class="pmagic"> 
  ...
  <div class="pm-group-view pm-dbfl pm-bg-lt">
   ...
   value="<?php _e('Save','profile-magic');?>" />
        <input type="button" name="expand" id="expand" value="<?php _e('Expand All','profile-magic');?>" onclick="pm_expand_all_conent()" />
        <input type="button" id="collapse" name="collapse" value="<?php _e('Collapse All','profile-magic');?>" onclick="pm_collapse_all_conent()" />
        </span></div>
      <div id="pm-accordion" class="pm-dbfl">
        <?php 
foreach($sections as $section):
    if($dbhandler->get_global_option_value('pm_is_editable_primary_email','0')==1)
    {
