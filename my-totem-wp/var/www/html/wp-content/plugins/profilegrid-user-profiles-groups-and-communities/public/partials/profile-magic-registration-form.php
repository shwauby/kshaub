...
if(isset($_POST['reg_form_submit']) || isset($_POST['pm_payment_method']))
{
	...
	{
		$errors = __('captcha failed','profilegrid-user-profiles-groups-and-communities');	
	}
}
else
{
	...
	if($check==false)
	{
		$message =  __('Require Useremail field','profilegrid-user-profiles-groups-and-communities');
	}
	elseif($is_group_limit==1)
	{
	...
	}elseif($pmrequests->profile_magic_check_paid_group($gid)>0)
        {
            ...
            if($message == 'disabled')
            {
                $message = __('Payment system is not configured to accept payments. Please configure at least one payment processor for this to work.','profilegrid-user-profiles-groups-and-communities');
            }
        }
	else
	{
